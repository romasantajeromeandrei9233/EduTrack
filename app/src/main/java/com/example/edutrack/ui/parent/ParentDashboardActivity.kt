...
class ParentDashboardActivity : AppCompatActivity() {
    ...
    private fun showLinkChildDialog() {
        ...
        input.hint = "Enter Invitation Code"

        AlertDialog.Builder(this)
            .setTitle("Link Child")
            .setMessage("Enter the invitation code provided by the teacher")
            .setView(input)
            .setPositiveButton("Link") { _, _ ->
                val code = input.text.toString()
                if (code.isNotBlank()) {
                    Toast.makeText(this, "Invitation code feature coming in M3", Toast.LENGTH_SHORT).show()
                }
            }
